...
export function WithdrawalsTable({ records }: { records: WithdrawalRecord[] }) {
  if (records.length === 0) {
    return (
      <div className="bg-slate-50 rounded-xl p-14 text-center border border-slate-100">
        <p className="text-slate-500 text-sm">لا توجد سجلات سحوبات بعد</p>
      </div>
    );
  }
          ...
          <tbody className="divide-y divide-slate-100">
            {records.map((rec) => (
              ...
              >
                ...
                {/* Method — payment details / wallet address */}
