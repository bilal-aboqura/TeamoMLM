...
export function DepositsTable({ requests }: { requests: DepositRequest[] }) {
  ...
  if (requests.length === 0) {
    return (
      <div className="bg-slate-50 rounded-xl shadow-[0_4px_20px_rgba(0,0,0,0.03)] p-14 text-center">
        ...
        <p className="text-slate-400 text-sm mt-1">لا توجد إيداعات معلقة حالياً</p>
      </div>
    );
  }
  ...
  return (
    <>
              ...
              {requests.map((req) => (
                ...
                >
                  ...
                  <td className="px-4 py-4">
                    <span className="inline-flex items-center px-2.5 py-1 rounded-lg bg-amber-50 text-amber-600 text-xs font-semibold">
                      {req.package_name}
                    </span>
                  </td>
      ...
      <ProofViewerModal
        ...
        title={`طلب إيداع — ${selected?.full_name ?? ""}`}
        onClose={() => setSelected(null)}
        onApprove={async () => {
          if (!selected) return { error: "لا يوجد طلب محدد" };
          const result = await approveDeposit(selected.id);
          if ("success" in result) router.refresh();
          return result;
        }}
        onReject={async (reason) => {
          if (!selected) return { error: "لا يوجد طلب محدد" };
          const result = await rejectDeposit(selected.id, reason);
          if ("success" in result) router.refresh();
          return result;
        }}
      />
